...
void nk_fpu_irq_record_usage(addr_t key) {
	...
	/*
	 * Take an IRQ lock. The critical section is pretty fast (and this is a debug feature, so...).
	 * TODO: in order to save time, we could do an atomic check on `current_session` and avoid taking
	 * the lock just to check if there is a session.
	 */
	...
	if (nk_htable_count(session->histogram) > 0) {
		...
		do {
			...
			if (prov != NULL) {
				...
				/* AFAIK, it's okay to just free this, as it's a container that points to static strings
...
nk_fpu_irq_session_t *nk_fpu_irq_end_session(void) {
	...
	/* Take an IRQ lock. The critical section is pretty fast, and this is a debug feature, so. */
	uint8_t flags = spin_lock_irq_save(&session_lock);
	/* TODO: since this is just a pointer, we could probaby do an atomic operation to remove it. */
...
void nk_thread_push_irq_frame(struct thread_debug_fpu_frame *frame) {
	...
	frame->state = malloc(FPU_STATE_SIZE);
...
#if defined(NAUT_CONFIG_FPU_IRQ_SAVE_LAZY) || defined(NAUT_CONFIG_FPU_IRQ_SAVE_RECORD)
